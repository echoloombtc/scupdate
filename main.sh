...
MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/kenDevXD/izinsc/main/ip | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
        TIMEDATE
    else
...
KYTPROJECT() {
    curl -sS https://raw.githubusercontent.com/kenDevXD/izinsc/main/ip >/root/tmp
...
function finish(){
    USRSC=$(curl -sS https://raw.githubusercontent.com/kenDevXD/izinsc/main/ip | grep $MYIP | awk '{print $2}')
    EXPSC=$(curl -sS https://raw.githubusercontent.com/kenDevXD/izinsc/main/ip | grep $MYIP | awk '{print $3}')
